...
// 直近20日の高値を更新したら「新しい上昇の始まり」とみなして買うブレイク
// アウト戦略。出来高で裏付けを取る。手仕舞いは20日安値割れ（トレーリング
// ストップ的な発想）。
  ...
  decide(position, s) {
    ...
    if (!hasPosition) {
      if (s.dayClose >= s.highestHigh20 && s.volumeRatio >= VOLUME_RATIO_MIN) {
        return { action: 'buy', budgetJpy: MAX_POSITION_YEN, reason: `20日高値更新（出来高${s.volumeRatio.toFixed(1)}倍で裏付け）を確認、ブレイクアウト買い` };
      }
