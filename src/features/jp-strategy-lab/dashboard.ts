...
import type { DaytradeCandidate } from '../daytrade-sim/types.ts';
...
// 2026年9月、ユーザー指摘「発注中の件数は分かるが、どの銘柄が売買待ちか分からない」を
// 受けて追加。daytrade-sim（機能6）のDashboardPendingOrderと同じ考え方だが、
// このラボは戦略ごとに別会計なので戦略IDをキーにしたRecordで持つ。
export interface DashboardPendingOrderLab {
  symbol: string;
  name: string;
  action: 'buy' | 'sell';
...
export interface DashboardDataLab {
  ...
  pendingOrders: Record<string, DashboardPendingOrderLab[]>;
}

/** Builds the comparison payload consumed by docs/jp-strategy-lab/index.html. Same shape as crypto-strategy-lab/us-strategy-lab's dashboard.ts (kept consistent even with only 1 strategy so far, so the dashboard doesn't need to change shape when more are added). */
export function buildDashboardData(
  ...
  const pendingOrders: Record<string, DashboardPendingOrderLab[]> = {};
  ...
  for (const strategy of strategies) {
    ...
    pendingOrders[strategy.id] = Object.entries(state.pendingOrders).map(([symbol, order]) => ({
      symbol,
      name: nameFor(symbol),
      action: order.action,
      quantity: order.quantity,
      limitPriceJpy: order.limitPriceJpy,
      reason: order.reason,
